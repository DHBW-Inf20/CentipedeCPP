//Quelle: https://stackoverflow.com/questions/421860/capture-characters-from-standard-input-without-waiting-for-enter-to-be-pressed
#include <iostream>
#include <string>
...
int key_press() { // not working: F11 (-122, toggles fullscreen)
    ...
    INPUT_RECORD irec;
    DWORD events;
    // Edit RE: Wir wollen die Polling Schleife nach aussen verlagern, um bei Programmende interrupten zu können.
    // while(true) {
    ReadConsoleInput(GetStdHandle(STD_INPUT_HANDLE), &irec, 1, &events);
    // Edit RE: Falls kein Tastendruck registriert return 0. Zusätzlich: Inverted if to reduce nesting.
